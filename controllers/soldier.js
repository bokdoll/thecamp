...
const controller = {
    getSoldier: async(req, res) => {
        ...
        for(var i=0; i<result.length; i++){
            temp_img = []
            const imgList = await soldierModel.getSoldierImage(result[i]["soldier_idx"]);
        ...
        return res.status(statusCode.OK)
            .send(util.success(statusCode.OK,responseMessage.READ_SOLDIER_SUCCESS,temp));
    }
}
